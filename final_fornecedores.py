...
class funcs():
    ...
    def new_fornecedor(self):
        ...
        self.conecta_bd()
        self.clean_tela()
        dados = " INSERT INTO fornecedores (bi, nome, telefone, morada) VALUES (%s, %s, %s, %s)"
    ...
    def select_lista(self):
        ...
        self.conecta_bd()
        self.cursor.execute(" SELECT bi, nome, telefone, morada FROM fornecedores ORDER BY nome ASC; ")
    ...
    def delete_fornecedor(self):
        ...
        self.conecta_bd() 
        print(self.bi)
        dados2 = "DELETE FROM fornecedores WHERE bi = {}" .format(self.bi)
        self.cursor.execute(dados2)
        self.conn.commit()
        self.desconecta_db()
        self.clean_tela()
        self.select_lista()
    def alter_fornecedor(self):
        self.variaveis()
        self.conecta_bd()
        dados = "UPDATE fornecedores SET bi = %s, nome = %s, telefone = %s, morada = %s"
        self.cursor.execute( dados, (self.bi, self.nome, self.telefone, self.morada))
        self.conn.commit()
        self.desconecta_db()
        self.select_lista()
        self.clean_tela()
    def search_fornecedor(self):
        self.conecta_bd()
        self.listaCli.delete(*self.listaCli.get_children())
        self.bi_entry.insert(END, '%')
        bi = self.bi_entry.get()
        self.cursor.execute("""SELECT bi, nome, telefone, morada FROM fornecedores WHERE bi LIKE '%s' ORDER BY bi ASC""" % bi)
...
class application(funcs):
    ...
    def __init__(self):
        ...
        self.lista_frame2()
        self.select_lista()
        fornecedores.mainloop()
    def tela(self):
        self.fornecedores.title("FORNECEDORES")
        self.fornecedores.configure(background= '#1e3743')
        self.fornecedores.geometry("800x600")
        self.fornecedores.resizable(True, True)
        self.fornecedores.maxsize(width= 1000, height= 800)
        self.fornecedores.minsize(width= 400, height= 300)
    def frames_da_tela(self):
        self.frame_1 = Frame(self.fornecedores, bd = 4, bg = '#dfe3ee',
                             highlightbackground= '#759fe6', highlightthickness= 3)
        self.frame_1.place(relx= 0.02, rely= 0.02, relwidth= 0.96, relheight= 0.46)

        self.frame_2 = Frame(self.fornecedores, bd = 4, bg = '#dfe3ee',
                             highlightbackground= '#759fe6', highlightthickness= 3)
    ...
    def widgets_frame1(self):
        ...
        self.bt_search = Button(self.frame_1, text= "SEARCH", bd= 2, bg= '#107db2', fg= 'white', font= ('verdana', 8, 'bold'), command= self.search_fornecedor)
        self.bt_search.place(relx= 0.3, rely= 0.1, relwidth= 0.1, relheight= 0.15)

         ### criação do botão NEW
        self.bt_new = Button(self.frame_1, text= "NEW", bd= 2, bg= '#107db2', fg= 'white', font= ('verdana', 8, 'bold'), command= self.new_fornecedor)
        self.bt_new.place(relx= 0.6, rely= 0.1, relwidth= 0.1, relheight= 0.15)

        ### criação do botão ALTER
        self.bt_alter = Button(self.frame_1, text= "ALTER", bd= 2, bg= '#107db2', fg= 'white', font= ('verdana', 8, 'bold'), command= self.alter_fornecedor)
        self.bt_alter.place(relx= 0.7, rely= 0.1, relwidth= 0.1, relheight= 0.15)
        
        ### criação do botão DELETE
        self.bt_delete = Button(self.frame_1, text= "DELETE", bd= 2, bg= '#107db2', fg= 'white', font= ('verdana', 8, 'bold'), command= self.delete_fornecedor)
